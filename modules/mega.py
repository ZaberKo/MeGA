...
class MeGA_Large(nn.Module):
    def __init__(self, model_config, num_classes=1000):
        super(MeGA_Large, self).__init__()
        ...
        for layer_config in model_config:
            ...
            mb_list.append(
                Block_Enhanced(kernel_size, in_size, round(in_size*expansion_rate), out_size,
...
class MeGA_Small(nn.Module):
    def __init__(self, model_config, num_classes=1000):
        super(MeGA_Small, self).__init__()
        self.stem = nn.Sequential(
            nn.Conv2d(3, 16, kernel_size=3, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(16),
            hswish(inplace=True)
